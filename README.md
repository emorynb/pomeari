...
This is all still very WIP (see #1 for details), so it's recommended that you only perform a toy install within the `uv` virtual environment.
...
* Add/remove configuration entries (including API secrets) using `pomeari config {add|rm} <key> [<value>]`\
  e.g. `pomeari config add mastodon_token uQL3g0C7h5wvcyQbt5vNFGPxt2tPTe`
...
* See the crossposts you did using `pomeari post logs [{-n|--max-count} <LIMIT>]`, or just `pomeari post logs`
